...
const termGlossary = {
  "ANOVA": "複数の群の平均に差があるかを、群間変動と群内変動の比で調べる分散分析です。",
  "CRT": "互いに素な法に関する連立合同式を、ひとつの合同式として解けることを述べる中国剰余定理です。",
  "Green の定理": "平面領域上の線積分を領域内部の二重積分に変換する定理です。",
  "p値": "帰無仮説が正しいとしたとき、観測結果以上に極端な結果が出る確率です。",
  "rank-nullity": "線形写像の次元について、定義域の次元が像の次元と核の次元の和になる定理です。",
  "Stokes の定理": "曲面の境界に沿う線積分を、曲面上の回転の面積分に変換する定理です。",
  "SVD": "行列を直交行列、特異値を並べた対角行列、直交行列の積に分解する特異値分解です。",
  "Sylow 定理": "有限群の位数に含まれる素数冪部分に対応する部分群の存在や個数を述べる定理です。",
  "イデアル": "環の部分集合で、加法について部分群になり、環の元を掛けても中に留まるものです。",
  "カイ二乗検定": "度数データの適合度や独立性を、カイ二乗分布を使って調べる検定です。",
  "ガロア拡大": "正規かつ分離的な体拡大で、ガロア群と中間体の対応を扱いやすい拡大です。",
  "ガロア群": "体拡大を保つ自己同型全体が合成で作る群です。",
  "グラフ曲面": "z=f(x,y) の形で表される曲面です。",
  "ジョルダン標準形": "複素数体上の行列を固有値ごとのジョルダンブロックに分解した標準形です。",
  "テイラー級数": "関数をある点の導関数の値から冪級数として表す展開です。",
  "データ可視化": "データの傾向、ばらつき、外れ値、関係性を図やグラフで把握する方法です。",
  "ノルム": "ベクトルや関数の大きさを表す非負の量です。",
  "はさみうち": "対象を同じ極限に近づく上下の関数や数列ではさみ、極限を求める方法です。",
  "パラメータ表示": "曲線や曲面を媒介変数を使って表す方法です。",
  "ピボット列": "行基本変形後に主成分を持つ列で、列空間の基底選択に使います。",
  "フラックス": "ベクトル場が曲面をどれだけ通り抜けるかを表す量です。",
  "ベイズの定理": "条件付き確率を反転し、観測後の確率を計算する公式です。",
  "ベイズ推定": "事前分布とデータの尤度から事後分布を求めて推定する方法です。",
  "ベクトル解析": "勾配、発散、回転、線積分、面積分などを扱う多変数解析の分野です。",
  "ベクトル計算": "ベクトルの和、スカラー倍、内積、外積などを計算する基本操作です。",
  "ベクトル恒等式": "勾配、発散、回転などの微分演算子に関する恒等式です。",
  "ベクトル場の線積分": "曲線に沿ってベクトル場の接線方向成分を積分する量です。",
  "ヘッセ行列": "多変数関数の二階偏導関数を並べた行列で、極値判定に使います。",
  "ポアソン分布": "一定の時間や領域でまれな事象が起こる回数を表す離散分布です。",
  "ポテンシャル": "勾配を取ると与えられた保存場になるスカラー関数です。",
  "モデル選択": "複数の統計モデルから、説明力と複雑さのバランスを考えて選ぶ手続きです。",
  "ラプラシアン": "スカラー場の勾配の発散で、二階偏微分の和として表される演算子です。",
  "一次独立": "係数を掛けた和が零になるのが自明な係数の場合だけであるベクトルの性質です。",
  "一様分布": "指定された範囲内の値が同じ密度または同じ確率で現れる分布です。",
  "陰関数": "方程式で暗黙に定まる関数で、明示的に y=f(x) と書かれていないものです。",
  "円柱座標": "空間の点を半径、角度、高さで表す座標系です。",
  "応用": "理論や計算手法を、具体的な問題設定や他分野のモデルに使うことです。",
  "仮説検定": "データから帰無仮説を棄却できるかを判断する統計的手続きです。",
  "加群": "環の元でスカラー倍できる、ベクトル空間を一般化した代数構造です。",
  "加群準同型": "加群の加法とスカラー倍を保つ写像です。",
  "可換群": "任意の二元の積の順序を入れ替えても結果が同じ群です。",
  "可逆行列": "積が単位行列になる逆行列を持つ正方行列です。",
  "解空間": "方程式や同次系を満たすすべての解からなる集合です。",
  "回帰": "説明変数から目的変数を予測・説明する統計モデルです。",
  "回帰の解釈": "係数、切片、残差、信頼区間などからモデルの意味を読み取ることです。",
  "回帰診断": "残差や外れ値、影響点などを確認し、回帰モデルの妥当性を調べることです。",
  "回転": "ベクトル場の局所的な渦の強さと向きを表す微分演算子です。",
  "回転なし": "ベクトル場の回転が零である性質で、単連結領域では保存場判定に使えます。",
  "階数": "行列や線形写像の像の次元で、独立な行または列の最大数です。",
  "階数・退化次数": "階数は像の次元、退化次数は核の次元で、rank-nullity で結ばれます。",
  "階数条件": "連立一次方程式の解の存在や一意性を係数行列と拡大係数行列の階数で判定する条件です。",
  "外積": "三次元で二つのベクトルに垂直なベクトルを作る演算です。",
  "拡大係数行列": "連立一次方程式の係数行列に右辺ベクトルを付け加えた行列です。",
  "核": "線形写像で零ベクトルへ写る元全体からなる部分空間です。",
  "確率の加法法則": "事象の和の確率を、重なりを調整して求める法則です。",
  "確率の基本性質": "確率が 0 以上 1 以下で、全事象が 1、互いに排反な和に加法性を持つ性質です。",
  "完全微分": "ある関数の全微分として表せる微分形式です。",
  "環": "加法と乗法を持ち、分配法則などを満たす代数構造です。",
  "環準同型": "環の加法、乗法、必要に応じて単位元を保つ写像です。",
  "関数の極限": "入力がある値に近づくとき、関数値が近づく値です。",
  "基底": "空間の任意の元を一意に線形結合で表せる一次独立な生成系です。",
  "基底変換": "同じベクトルや線形写像を、別の基底に関する座標で表し直すことです。",
  "基本極限": "三角関数や指数・対数などの計算で頻繁に使う標準的な極限です。",
  "基本定理": "分野の中心となる対象間の対応や構造を述べる定理です。",
  "基本部分空間": "行列に付随する列空間、行空間、核、左零空間などの部分空間です。",
  "幾何分布": "独立なベルヌーイ試行で初めて成功するまでの回数を表す分布です。",
  "既約性": "多項式や元が非自明な積に分解できない性質です。",
  "期待値": "確率変数の平均的な値で、値を確率で重み付けした和または積分です。",
  "擬似逆行列": "正方でない行列や階数落ち行列にも使える逆行列の一般化です。",
  "逆行列": "元の行列との積が単位行列になる行列です。",
  "球座標": "空間の点を半径、方位角、極角で表す座標系です。",
  "級数の性質": "級数の収束、発散、線形性、項別操作などに関する性質です。",
  "級数の和": "無限個の項を足し合わせた極限として定義される値です。",
  "共分散": "二つの確率変数が同じ方向に変動する傾向を表す量です。",
  "共役": "群の元 g に対して x を gxg^{-1} の形に移す操作です。",
  "共役類": "互いに共役な元を集めた集合です。",
  "曲線の向き": "線積分で曲線をたどる方向を指定する情報です。",
  "曲線座標の微分": "極座標、円柱座標、球座標などで微分演算子を表す方法です。",
  "曲面のパラメータ表示": "二つの媒介変数で曲面上の点を表す方法です。",
  "極限定理": "標本サイズや試行回数を大きくしたときの確率変数や分布の極限を述べる定理です。",
  "極座標": "平面上の点を原点からの距離と角度で表す座標系です。",
  "極座標積分": "極座標変換を使い、ヤコビアン r を掛けて積分する方法です。",
  "区分的滑らかな曲線": "有限個の区間に分けると各区間で滑らかな曲線です。",
  "群": "結合法則、単位元、逆元を持つ二項演算付き集合です。",
  "群作用": "群の元が集合の変換として働き、群演算と両立する構造です。",
  "経路独立": "線積分の値が始点と終点だけで決まり、途中の経路によらない性質です。",
  "検定の誤り": "帰無仮説を誤って棄却する第一種の誤りと、誤って棄却しない第二種の誤りです。",
  "検定の種類": "平均、比率、分散、適合度、独立性など、目的に応じた検定の分類です。",
  "検定の性質": "有意水準、検出力、p値、棄却域など検定の性能や判断基準を表す性質です。",
  "厳密な定義": "極限や連続性などを、ε-δ 論法のように量化して定める定義です。",
  "固有ベクトル": "線形写像で向きが変わらず、スカラー倍される非零ベクトルです。",
  "固有空間": "ある固有値に対応する固有ベクトルと零ベクトル全体の空間です。",
  "固有多項式": "det(λI-A) で定義され、固有値を根に持つ多項式です。",
  "固有値": "線形写像がある非零ベクトルをスカラー倍に写すときのスカラーです。",
  "固有値の性質": "固有値と行列式、跡、対角化、固有空間などの関係です。",
  "弧長": "曲線の長さで、速度ベクトルのノルムを積分して求めます。",
  "勾配": "スカラー場の最も増加する方向と増加率を表すベクトルです。",
  "勾配の意味": "勾配は等位面に垂直で、関数が最も速く増える方向を示します。",
  "向き付け": "曲線や曲面に積分の正方向を指定することです。",
  "広義積分": "無限区間や特異点を含む積分を極限で定義したものです。",
  "行空間": "行列の行ベクトルが張る部分空間です。",
  "行列計算": "行列の和、積、スカラー倍などの基本演算です。",
  "行列式": "正方行列に対応するスカラーで、体積倍率や可逆性を表します。",
  "行列表示": "線形写像を基底に関する行列として表すことです。",
  "行列分解": "行列を扱いやすい複数の行列の積に分けることです。",
  "高階偏微分": "多変数関数を同じ変数または複数の変数で二回以上偏微分したものです。",
  "左零空間": "A の転置の核で、A の列空間に直交する空間です。",
  "座標表示": "ベクトルを選んだ基底に関する係数の組で表すことです。",
  "最小二乗": "残差平方和を最小にするように近似解やモデル係数を求める方法です。",
  "最尤推定": "観測データが最も起こりやすくなるようにパラメータを選ぶ推定方法です。",
  "三重積分": "三次元領域上で関数を積分する操作です。",
  "仕事積分": "力のベクトル場に沿って移動したときの仕事を線積分で表したものです。",
  "四分位範囲": "第3四分位数と第1四分位数の差で、データの中央部分のばらつきを表します。",
  "指数分布": "ポアソン過程で次の事象が起こるまでの待ち時間を表す連続分布です。",
  "次元": "ベクトル空間の基底に含まれるベクトルの本数です。",
  "次元公式": "部分空間の和や線形写像について、次元同士の関係を表す公式です。",
  "自己同型": "構造を保つ同型写像のうち、対象からそれ自身への写像です。",
  "自由変数": "連立一次方程式の解で自由に値を選べる変数です。",
  "射影": "対象をある部分空間や方向へ写し落とす写像です。",
  "収束判定": "数列、級数、積分などが有限の値に近づくかを調べる方法です。",
  "重複度": "固有値や根が多項式に何回現れるかを表す数です。",
  "循環": "閉曲線に沿ったベクトル場の線積分で、周回方向の流れを表します。",
  "準同型": "群、環、加群などの演算や構造を保つ写像です。",
  "巡回群": "一つの元の冪や倍数だけで全体を生成できる群です。",
  "商群": "正規部分群で同じ剰余類をまとめて作る群です。",
  "剰余環": "環をイデアルで割り、剰余類を元として作る環です。",
  "場の種類": "スカラー場やベクトル場など、各点にどの種類の量を対応させるかの分類です。",
  "条件付き確率": "ある事象が起きたという条件のもとで別の事象が起きる確率です。",
  "条件付き期待値": "条件として与えた情報のもとでの確率変数の平均値です。",
  "信頼区間": "母数を含むと期待される範囲を、信頼係数とともにデータから作った区間です。",
  "推定量の性質": "不偏性、一致性、有効性など、推定量の良さを表す性質です。",
  "数え上げ": "順列、組合せ、重複、包除などを使って場合の数を求めることです。",
  "数値線形代数": "計算機で線形代数の問題を安定かつ効率よく解く方法を扱う分野です。",
  "数列の極限": "添字が大きくなるとき、数列の項が近づく値です。",
  "制約付き極値": "条件式のもとで関数の最大値や最小値を求める問題です。",
  "整域": "零因子を持たない可換環です。",
  "正規近似": "二項分布などを正規分布で近似して計算を簡単にする方法です。",
  "正規性": "体拡大で既約多項式が根を一つ持てばすべての根を持つ性質です。",
  "正規直交化": "一次独立なベクトル列から正規直交基底を作る手続きです。",
  "正規直交基底": "互いに直交し、それぞれのノルムが 1 の基底です。",
  "正規部分群": "任意の共役で自分自身に移る部分群で、商群を作るために必要です。",
  "正規分布": "平均を中心に左右対称なベル型の連続分布です。",
  "正定値": "零でない任意のベクトルに対して二次形式が正になる性質です。",
  "生成系": "線形結合によって空間全体を作れるベクトルの集まりです。",
  "跡": "正方行列の対角成分の和です。",
  "接線": "曲線上の一点で曲線に最もよく近似する直線です。",
  "接平面": "曲面上の一点で曲面に最もよく近似する平面です。",
  "線形結合": "ベクトルに係数を掛けて足し合わせたものです。",
  "線形写像": "加法とスカラー倍を保つ写像です。",
  "線形性": "加法とスカラー倍に関する演算を保つ性質です。",
  "線積分": "曲線に沿ってスカラー場やベクトル場を積分する操作です。",
  "全確率": "標本空間を分割する事象を使って、目的の事象の確率を足し合わせる公式です。",
  "全微分": "多変数関数の微小変化を各偏微分と変数の微小変化で一次近似したものです。",
  "双対空間": "ベクトル空間から基礎体への線形汎関数全体の空間です。",
  "掃き出し法": "行基本変形で行列を簡単な形にし、連立方程式や逆行列を求める方法です。",
  "相関": "二つの量が一緒に増減する傾向です。",
  "相関係数": "二つの変数の線形関係の強さと向きを -1 から 1 の値で表す量です。",
  "相似": "P^{-1}AP の形で行列を変換し、同じ線形写像を別の基底で表す関係です。",
  "像": "写像が実際に取りうる値全体の集合です。",
  "増減と凹凸": "導関数と二階導関数から関数の増減、極値、曲がり方を調べることです。",
  "多項式環": "係数をある環や体から取り、多項式を元とする環です。",
  "体": "零でない元がすべて逆元を持つ可換環です。",
  "体拡大": "小さい体を含む大きい体を考える構造です。",
  "体自己同型": "体の加法、乗法、単位元を保つ、自分自身への同型写像です。",
  "対角化": "行列を適当な基底で対角行列に相似変換することです。",
  "対称行列": "転置しても変わらない正方行列です。",
  "対称性": "図形、領域、関数、分布などが変換しても変わらない性質です。",
  "代数拡大": "拡大体の各元が基礎体上の多項式の根になる体拡大です。",
  "代表値": "平均、中央値、最頻値など、データの中心を表す値です。",
  "単元": "環の中で乗法逆元を持つ元です。",
  "置換群": "集合の置換を合成で演算する群です。",
  "置換積分": "変数を置き換えて積分を計算しやすくする方法です。",
  "中心": "群のすべての元と可換な元全体からなる部分群です。",
  "中心極限定理": "独立な確率変数の和や平均が、条件のもとで正規分布に近づく定理です。",
  "超幾何分布": "有限集団から非復元抽出したときの成功数を表す離散分布です。",
  "直交": "内積が 0 であることです。",
  "直交行列": "列ベクトルが正規直交基底をなし、転置が逆行列になる行列です。",
  "直交射影": "ベクトルを部分空間へ垂直に落とす射影です。",
  "直交補": "部分空間のすべてのベクトルと直交するベクトル全体の空間です。",
  "直積": "複数の群や集合を成分ごとに組み合わせて作る構造です。",
  "定積分": "区間上の関数の符号付き面積や累積量を表す積分です。",
  "転置行列": "行と列を入れ替えた行列です。",
  "点推定": "データから母数を一つの値として推定することです。",
  "同型": "構造を完全に保つ全単射です。",
  "同型定理": "準同型の像と核による商構造の関係を述べる定理です。",
  "同時分布": "複数の確率変数が同時に取る値の確率分布です。",
  "同次系": "右辺が零ベクトルの連立一次方程式です。",
  "導関数": "関数の瞬間的な変化率を表す関数です。",
  "独立性": "一方の事象や確率変数の情報が他方の確率に影響しない性質です。",
  "内積": "二つのベクトルからスカラーを返し、長さや角度を測る演算です。",
  "二項分布": "独立なベルヌーイ試行で成功回数を表す離散分布です。",
  "二次形式": "ベクトル x に対して x^T A x の形で表される二次式です。",
  "二重積分": "平面領域上で関数を積分する操作です。",
  "発散": "ベクトル場が一点からどれだけ湧き出すかを表す微分演算子です。",
  "発散定理": "閉曲面を通るフラックスを、その内部領域での発散の三重積分に変換する定理です。",
  "比率の検定": "母比率や二つの比率の差について仮説を検定する手続きです。",
  "微分演算子": "関数や場に微分を作用させる演算子です。",
  "微分法": "導関数を求め、関数の変化を調べる方法です。",
  "標準形": "対象を分類や計算に便利な代表的な形に変換したものです。",
  "標数": "環や体で 1 を何回足すと 0 になるかを表す数です。",
  "標本空間": "試行で起こりうるすべての結果を集めた集合です。",
  "標本比率": "標本内で条件を満たす個体の割合です。",
  "標本分布": "標本から計算した統計量が、標本抽出を繰り返したときに従う分布です。",
  "標本平均": "標本データの値を足して標本数で割った平均です。",
  "不定形": "極限計算で直接代入すると値が決まらない形です。",
  "不定形の解消": "変形、ロピタルの定理、標準極限などで不定形を計算可能な形にすることです。",
  "不定積分": "導関数が与えられた関数になる原始関数の全体です。",
  "部分空間": "加法とスカラー倍で閉じたベクトル空間の部分集合です。",
  "部分群": "群の部分集合で、それ自身も同じ演算で群になるものです。",
  "部分積分": "積の微分公式から導かれる積分計算法です。",
  "分解体": "多項式が一次式の積に完全に分解する最小の体拡大です。",
  "分散": "確率変数やデータが平均からどれだけ散らばるかを表す量です。",
  "分離性": "多項式が重根を持たないこと、または拡大がその性質を持つことです。",
  "平均": "データや確率分布の中心を表す値です。",
  "平均の検定": "母平均や二つの平均差について仮説を検定する手続きです。",
  "閉曲面": "境界を持たず、内部領域を囲む曲面です。",
  "偏微分": "多変数関数で一つの変数だけを動かして微分することです。",
  "変数変換": "座標や変数を置き換えて、積分や方程式を扱いやすくする方法です。",
  "片側極限": "左側または右側の一方向から近づくときの極限です。",
  "保存場": "あるポテンシャルの勾配として表せ、線積分が経路によらないベクトル場です。",
  "母関数": "数列や確率分布を係数として持つ級数で、性質や計算をまとめて扱う道具です。",
  "方向微分": "指定した方向へ動いたときの関数の変化率です。",
  "法線ベクトル": "曲線や曲面に垂直なベクトルです。",
  "面積": "平面領域の大きさで、積分を使って求められます。",
  "面積公式": "線積分や変数変換などを使って領域の面積を求める公式です。",
  "面積分": "曲面上でスカラー場やベクトル場を積分する操作です。",
  "有意水準": "第一種の誤りを許す上限として事前に決める確率です。",
  "有限体": "有限個の元からなる体です。",
  "有理関数の積分": "多項式の比で表される関数を部分分数分解などで積分する方法です。",
  "領域の位相": "単連結性や穴の有無など、保存場や積分定理の適用に関わる領域の性質です。",
  "累次積分": "多重積分を一変数ずつ順番に積分して計算する形です。",
  "列空間": "行列の列ベクトルが張る部分空間です。",
  "連鎖律": "合成関数の微分を内側と外側の微分の積で求める法則です。",
  "連続関数の性質": "中間値の定理、最大最小の定理など、連続性から従う基本性質です。",
  "連立一次方程式": "複数の一次方程式を同時に満たす変数を求める問題です。",
  "冪級数": "変数の冪を係数付きで足し合わせた級数です。"
};

const termEnglishNames = {
  "ANOVA": "analysis of variance",
  "CRT": "Chinese remainder theorem",
  "Green の定理": "Green's theorem",
  "p値": "p-value",
  "rank-nullity": "rank-nullity theorem",
  "Stokes の定理": "Stokes' theorem",
  ...
  "ノルム": "norm",
  "はさみうち": "squeeze theorem",
  "パラメータ表示": "parametric representation",
  "ピボット列": "pivot column",
  "フラックス": "flux",
  "ベイズの定理": "Bayes' theorem",
  "ベイズ推定": "Bayesian estimation",
  "ベクトル解析": "vector calculus",
  "ベクトル計算": "vector operations",
  "ベクトル恒等式": "vector identity",
  "ベクトル場の線積分": "line integral of a vector field",
  "ヘッセ行列": "Hessian matrix",
  "ポアソン分布": "Poisson distribution",
  "ポテンシャル": "potential function",
  "モデル選択": "model selection",
  "ラプラシアン": "Laplacian",
  "一次独立": "linear independence",
  "一様分布": "uniform distribution",
  "陰関数": "implicit function",
  "円柱座標": "cylindrical coordinates",
  "応用": "application",
  "仮説検定": "hypothesis testing",
  "加群": "module",
  "加群準同型": "module homomorphism",
  "可換群": "abelian group",
  "可逆行列": "invertible matrix",
  "解空間": "solution space",
  "回帰": "regression",
  "回帰の解釈": "regression interpretation",
  "回帰診断": "regression diagnostics",
  "回転": "curl",
  "回転なし": "irrotational",
  "階数": "rank",
  "階数・退化次数": "rank and nullity",
  "階数条件": "rank condition",
  "外積": "cross product",
  "拡大係数行列": "augmented matrix",
  "核": "kernel",
  "確率の加法法則": "addition rule of probability",
  "確率の基本性質": "basic properties of probability",
  "完全微分": "exact differential",
  "環": "ring",
  ...
  "級数の性質": "properties of series",
  "級数の和": "sum of a series",
  "共分散": "covariance",
  "共役": "conjugation",
  "共役類": "conjugacy class",
  "曲線の向き": "orientation of a curve",
  "曲線座標の微分": "differentiation in curvilinear coordinates",
  "曲面のパラメータ表示": "parametric representation of a surface",
  "極限定理": "limit theorem",
  "極座標": "polar coordinates",
  "極座標積分": "integration in polar coordinates",
  "区分的滑らかな曲線": "piecewise smooth curve",
  "群": "group",
  "群作用": "group action",
  "経路独立": "path independence",
  "検定の誤り": "errors in hypothesis testing",
  "検定の種類": "types of hypothesis tests",
  "検定の性質": "properties of hypothesis tests",
  "厳密な定義": "rigorous definition",
  "固有ベクトル": "eigenvector",
  "固有空間": "eigenspace",
  "固有多項式": "characteristic polynomial",
  "固有値": "eigenvalue",
  "固有値の性質": "properties of eigenvalues",
  "弧長": "arc length",
  "勾配": "gradient",
  "勾配の意味": "meaning of the gradient",
  "向き付け": "orientation",
  ...
  "高階偏微分": "higher-order partial derivative",
  ...
  "相似": "similarity",
  "像": "image",
  "増減と凹凸": "monotonicity and concavity",
  "多項式環": "polynomial ring",
  "体": "field",
  "体拡大": "field extension",
  "体自己同型": "field automorphism",
  "対角化": "diagonalization",
  ...
  "不定形の解消": "resolving indeterminate forms",
  ...
  "分離性": "separability",
  "平均": "mean",
  "平均の検定": "test for means",
  "閉曲面": "closed surface",
  "偏微分": "partial derivative",
  "変数変換": "change of variables",
  "片側極限": "one-sided limit",
  "保存場": "conservative vector field",
  ...
  "領域の位相": "topology of a domain",
  "累次積分": "iterated integral",
  "列空間": "column space",
  "連鎖律": "chain rule",
  "連続関数の性質": "properties of continuous functions",
...
function normalizeTerms(problem) {
  ...
  return rawTerms.map((term, index) => {
    const fallbackId = `${problem.id}-term-${index}`;
    if (typeof term === "string") {
      const glossaryDescription = termGlossary[term];
      const englishName = termEnglishNames[term];
      return {
        id: fallbackId,
        name: term,
        englishName,
        description: glossaryDescription ?? defaultTermDescription
      };
    }

    const name = term.name ?? term.title ?? problem.topic ?? "Term";
    return {
      id: term.id ?? fallbackId,
      name,
      englishName: term.englishName ?? term.english ?? termEnglishNames[name],
      description: term.description ?? term.definition ?? termGlossary[name] ?? defaultTermDescription
    };
  });
}
        ...
        .map(
          (term) => `
            <article id="${escapeHtml(term.id)}" class="term-card" tabindex="-1">
              ...
              ${term.englishName ? `<div class="term-english">${escapeHtml(term.englishName)}</div>` : ""}
